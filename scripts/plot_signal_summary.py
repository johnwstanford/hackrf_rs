...
def plot_freq_domain(data, center_freq, rate_sps, marker, exclude_dc=True):
    ...
    all_freqs = freqs_negative + freqs_positive
    if exclude_dc:
        all_freqs = list(filter(lambda f: abs(f['freq']) > 0, all_freqs))
